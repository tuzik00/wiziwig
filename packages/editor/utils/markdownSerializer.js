...
const getBlockStyle = (currentStyle, appliedBlockStyles) => {
    if (currentStyle === BLOCK_TYPE.OL) {
        const counter = appliedBlockStyles.reduce((prev, style) => {
            if (style === BLOCK_TYPE.OL) {
                return prev + 1;
            }
...
function draftjsToMd(raw, extraMarkdownDict) {
    ...
    return raw.blocks.reduce((accum, block) => {
        switch (block.type) {
            ...
            default:
                const node = accum[accum.length - 1];

                if (node && node.type === BLOCK_TYPE.MARKDOWN) {
                    node.data += '\n' + gen(block);

                    accum[accum.length - 1] = node;
                } else {
                    if (!block.text) {
                        return accum;
                    }
